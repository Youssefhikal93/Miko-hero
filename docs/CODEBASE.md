...
### Encrypted files — `core/files/`

| File | Responsibility |
| --- | --- |
| `encrypted_file_picker.dart` | The one platform boundary every file this app offers travels through: `EncryptedFilePicker` (pick with a size cap, save under a name), the `file_picker` adapter, and `safeFileStem`, the single naming rule — strip what no file system accepts, bound it to 60 runes, fall back rather than hand a platform an empty name. Backups, story files, and rendered PDFs all use it, so a test replaces exactly one thing to drive any of them. |
| `encrypted_file_flow.dart` | `EncryptedFileFlow<T>`: encrypt a payload to a file and open one back, configured per payload type by codec, extension, size cap, and naming rule. `export` reports saved or cancelled; `import` picks, asks for the password through a callback, and decodes, returning null when the parent dismissed either step. Every codec and picker failure arrives as one `EncryptedFileException` carrying an `EncryptedFileFailure`, so a screen looks up one sentence per reason instead of switching over crypto classes. |
...
| `profile_page.dart` | Profile list, each child led by their shared `HeroFace`, and the editor form (name, localized birth-date picker bounded to ages 1–17, Girl/Boy choice, reference photo). The editor's own square preview stays its own: it shows the photo being picked, which is not yet any child's face. |
...
| `kingdom_decorations.dart` | Every kingdom decoration drawn locally: `KingdomCastle` and `KingdomAvatar`, which owns only the painted frame (`CustomPaint`) and draws the face inside it with the shared `HeroFace`; the backdrop gradient, and the bounded Material icon for each favourite symbol. Adds no binary asset and makes no network call. |
...
| File | Responsibility |
| --- | --- |
| `story_creation_page.dart` | Tap-first request form: hero cards carrying the shared `HeroFace` (plus the existing add-profile route), inherited Girl/Boy context with the shared selector only for legacy profiles missing it, plain theme and moral fields, 6/8/10 page segments, illustration-style cards swatched from the shared `story_artwork.dart` table so the card previews the artwork the chosen hero will actually get, and self-scripted language chips. Its header waits for the saved selection before naming the actual Demo or Local AI generator, the selected child's saved preferences and safety-exclusion count stay visible, and the unchanged progress panel follows the paired PC stage by stage. |
...
| `story_share_controller.dart` | The two decisions only this app can make about a story file: whose display name travels with the book (never the child's photo), and that an imported one joins a chosen shelf through `LibraryTransaction`, which is what puts it in shelf order. Naming, size cap, picking, and every typed failure belong to the single-story configuration of `EncryptedFileFlow` it hosts. Refuses an already present story identity (`DuplicateStoryException`) and a destination profile that no longer exists (`UnknownEntityException`). |
| `story_share_actions.dart` | The parent-gated export and import flows: password prompt, decrypted preview (title, page count, hero), and destination-profile choice. Talks only to `StoryShareController`, and hands every failure to `storyFileMessage` rather than knowing what a codec or a picker can raise. |
...
| File | Responsibility |
| --- | --- |
| `story_reader_page.dart` | Full-screen reader, arranged as the redesign's Reader screen: the hero's ringed `HeroFace`, an exit, and the session-only bedtime toggle in a top row; swipeable pages whose picture, or the shared `story_artwork.dart` placeholder standing in for it until ComfyUI, fills the frame with the spoken sentence tinted in place; then one control row of previous, the wide candle "Read to me" control (play, pause, resume, plus a stop control while narration runs) and next; then "Page x of y" beside one dot per page, both following swipes and button turns; then one icon row of reading speed, sleep timer, text size, and PDF export. Speed and sleep timer open the shared narration dialog that also owns spoken scope; text size writes the hero's saved reading comfort through `ProfileController`; export asks whether the cover carries the child's photo. Arabic keeps its right-to-left story layout with previous and next mirrored. Reaching the last page records the story as finished once per session and celebrates a newly earned badge. Three short rows rather than one long one, so a 360 px phone keeps full touch targets without wrapping. Only approved stories can be opened. |
...
| `story_export_controller.dart` | Coordinates PDF rendering and the shared save dialog — the only file this app offers that carries no codec, so it takes the naming rule and the picker and nothing else; refuses to export unapproved drafts and resolves the hero's photo only when the parent opted in. |
...
| `backup_controller.dart` | The two decisions only this app can make about a backup: that it contains the current snapshot, and that restoring one replaces family state through `LibraryTransaction.replaceState` and drops the durable generation queue with it. Hosts the whole-family configuration of `EncryptedFileFlow`, which owns the dated file name, the size cap, picking, and every typed failure. |
| `backup_settings_card.dart` | Backup UI: password dialogs with confirmation, the browser-safe explicit save step, and the restore preview. Every failure is localized by `backupFileMessage` from its typed reason, including a backup written by a newer app version. |
...
| `encrypted_file_messages.dart` | The parent-facing sentence for each `EncryptedFileFailure`, once for backups and once for story files, so neither the card nor the share flow switches over codec or picker classes. A story already on this device is answered here too, because it is the one failure that is not about the file. |
...
| `hero_face.dart` | `HeroFace`, the one child's-photo-in-a-circle every screen draws: Home's header and switcher, the reader's hero and its page placeholder, the creation form's hero cards, the shelf's child chips, the kingdom avatar inside its painted frame, and the profile list. Decodes the stored photo once and never fails on it — an empty photo, base64 that will not decode, and bytes that are not an image all end at the child's initial (read as a whole grapheme, so an emoji or non-Latin name keeps its glyph) or at the glyph a surface asked for instead. The accent is the child's own saved colour; the ring, the surface behind an empty face, and the fallback glyph are each a surface's choice. |
...
| `story_card.dart` | The story tile in three shapes: `StoryCardVariant.large` (cover, title, page count and date, favourite heart, Demo badge), `small` (cover and title) and `wide` (cover thumbnail, title, meta, overflow). Every variant is one full-size tap target that opens the story, and the large and wide shapes carry the favorite, collections, illustrate, share, and delete commands their surrounding feature allows in a single overflow menu, each running the feature's own callback so parent gating stays where it lives. The card renders only what it is given: a feature that allows nothing gets no overflow control. Cover artwork — the drawn picture or the placeholder behind it — comes from `story_artwork.dart` rather than from a palette of its own. Also `StoryCover`, the single-book cover the parent review screen uses, and `StoryDemoBadge`. |
| `story_artwork.dart` | What a story looks like, decided once: `placeholderColors` is the one hue table over illustration style and hero gender, `gradientOf` reads it for a stored book, `swatchFor` is the same table the creation form previews a style with, and `coverOf`/`pageOf` resolve a page's drawn bytes through the shared per-page provider. The shelf tile, the reader page, and the style cards all read here, so a girl can no longer tap a cyan swatch and receive rose artwork, and the cover and the page can no longer close their colorful-3D gradient on different violets. Print colours stay separate in `StoryPdfPalette`, which shares no hue with these. |
...
| `test/core/files/encrypted_file_flow_test.dart` | The one flow backups and story files share, with the real codecs and a fake picker: an exported story reopens under a name a file system accepts, a title of slashes and angle brackets is bounded to 60 runes and a punctuation-only one falls back, a full backup opened as a story file is refused as unsupported and told apart from a wrong password, a selection over the cap is refused before it is decoded, dismissing the picker never asks for a password, dismissing either step imports nothing, cancelling the save writes nothing, and a backup travels out and back through the same flow. |
| `test/features/settings/backup_controller_test.dart` | The backup path above the codec, with real persistence: a backup exported here restores onto a device holding a different family and survives reopening the app, a wrong password restores nothing and names its reason, and dismissing the picker leaves the device untouched. |
| `test/support/fake_encrypted_file_picker.dart` | The single platform boundary every file flow needs replaced, applying the same size cap the real picker does and recording what a parent would have received. |
...
| `test/features/library/story_share_test.dart` | Importing writes the story to the chosen profile only, the same story twice is refused, an export carries the local hero name and no photo under the story's own file name, and through the real library UI a picked file is previewed, imported, and opened in the reader while a full backup is refused with a localized message. |
...
| PDF export (storybook layout, fonts, RTL) | `core/export/story_pdf_service.dart`, `story_pdf_layout.dart`, `story_pdf_symbols.dart`, `core/files/encrypted_file_picker.dart`, `features/reader/story_export_controller.dart`, `assets/fonts/` |
...
| Encrypted backup and restore | `core/backup/*`, `core/files/*`, `features/settings/backup_controller.dart`, `backup_settings_card.dart`, `shared/encryption_password_dialog.dart`, `shared/encrypted_file_messages.dart` |
| Single-story sharing | `core/models/shared_story.dart`, `core/backup/story_share_codec.dart`, `core/files/encrypted_file_flow.dart`, `features/library/story_share_controller.dart`, `story_share_actions.dart` |
