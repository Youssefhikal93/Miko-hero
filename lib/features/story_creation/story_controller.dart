...
class StoryController {
  ...
  Future<void> setCollections(String storyId, List<String> collections) {
    return _library.updateStory(
      storyId,
      (story) => story.withCollections(collections),
    );
  }
  ...
  /// The one seam that writes the library and then publishes it.
  ...
  Future<StoryBook> _generateJob(
    ...
    final completed = _currentState.storyById(storyId);
    if (completed != null) {
    ...
    try {
      ...
      await _library.mutateStories((stories) => <StoryBook>[story, ...stories]);
      await queue.remove(job.id);
      return story;
