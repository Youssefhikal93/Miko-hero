...
class _GatedOllamaCall {
  ...
  /// Answer returned once the page pass is released.
  final String payload;

  /// Answer returned once the outline pass is released.
...
void main() {
    ...
    test(
      ...
      () async {
        ...
        final client = FakeOllamaStoryClient.writing(
          story: storyPayload(pageCount: pageCount),
          pageCount: pageCount,
        );
  ...
  test(
    ...
    () async {
      ...
      expect(
        outlineCall.prompt,
        contains('فصحى مبسطة'),
        reason: 'the Arabic outline must be planned in fusha too',
      );

      final call = client.pageRequests.single;
      ...
      expect(
        call.prompt,
        contains(arabicTitle),
        reason: 'the approved outline travels into the page pass',
      );
  ...
  final malformedAnswers = <String, String>{
    ...
    'pages in the wrong language': storyPayload(
      pageCount: 6,
      text: (pageNumber) => 'أشعل نور فانوسًا صغيرًا وابتسم ($pageNumber).',
    ),
  };
  malformedAnswers.forEach((description, payload) {
    test(
      ...
      () async {
        final printedCodes = <String>[];
        final client = FakeOllamaStoryClient.writing(story: payload);
        final testServer = await createTestServer(
          ollamaClient: client,
          notifyCode: printedCodes.add,
        );
        ...
        expect(
          ...
          reason: 'the page pass is retried on every attempt',
        );
        expect(
          client.outlineRequests,
          hasLength(1),
          reason: 'the approved outline is reused instead of re-planned',
        );
        final jobBody = await readJob(testServer, token, jobId);
  ...
  final malformedOutlines = <String, String>{
    'an outline with the wrong beat count': outlinePayload(pageCount: 4),
    ...
    'an outline in the wrong language': outlinePayload(
      pageCount: 6,
      title: 'نور وفوانيس البحر',
      summary: (pageNumber) => 'مشى نور خطوة أخرى ($pageNumber).',
    ),
  };
  malformedOutlines.forEach((description, plan) {
    test('$description fails before any page is written', () async {
      final printedCodes = <String>[];
      final client = FakeOllamaStoryClient.writing(
        outline: plan,
      ...
      expect(
        client.outlineRequests,
        hasLength(testServer.server.config.maxGenerationAttempts),
        reason: 'a bad plan is re-planned on every attempt',
      );
      expect(
        client.pageRequests,
        isEmpty,
        reason: 'pass two never runs without an approved outline',
      );
      final jobBody = await readJob(testServer, token, jobId);
      final error = jobBody['error']! as Map<String, Object?>;
      expect(error['code'], 'invalid_model_output');
    ...
    () async {
      ...
      final client = FakeOllamaStoryClient((request, cancellation) async {
        ...
        return ollamaEnvelope(
          pageCalls == 1 ? 'not a story at all' : storyPayload(pageCount: 6),
        );
      });
      final testServer = await createTestServer(
        ollamaClient: client,
      ...
      expect(settled.status, GenerationJobStatus.completed);
      expect(client.outlineRequests, hasLength(1));
      expect(client.pageRequests, hasLength(2));
      expect(testServer.countRows('stories'), 1);
      expect(testServer.countRows('story_pages'), 6);
    },
  );

  test('the hero appearance sheet reaches every stored scene', () async {
    const appearance =
        'short curly black hair, mustard-yellow raincoat, red boots, '
        'carries a small brass lantern';
    final printedCodes = <String>[];
    final client = FakeOllamaStoryClient.writing(
      outline: outlinePayload(pageCount: 6, heroAppearance: appearance),
      story: storyPayload(pageCount: 6),
    );
    final testServer = await createTestServer(
      ollamaClient: client,
    ...
    final scenes = testServer.library.database
        .select('SELECT scene_description FROM story_pages ORDER BY page_index')
        .map((row) => row['scene_description']! as String)
        .toList();
    ...
    for (final scene in scenes) {
      expect(
        scene,
        contains(appearance),
        reason: 'every page must draw the same child in the same clothes',
      );
      expect(
        scene,
        startsWith('A child on a moonlit beach'),
        reason: 'the scene itself still leads',
      );
    }
  });

  test('the saved preferences of a child reach both prompts', () async {
    final printedCodes = <String>[];
    final client = FakeOllamaStoryClient.writing(
      story: storyPayload(pageCount: 6),
    );
    ...
    for (final call in client.requests) {
      expect(call.prompt, contains('sea turtles and paper boats'));
  ...
  test('a request without preferences says nothing about them', () async {
    ...
    for (final call in client.requests) {
      ...
      expect(call.prompt, isNot(contains('Things Nour loves')));
    }
  });
  ...
  test('a queued job can be cancelled before it ever reaches Ollama', () async {
    ...
    expect(
      ...
      hasLength(2),
      reason:
          'only the running job planned and wrote; the cancelled one '
  ...
  test('a second job waits its turn and reports its queue position', () async {
    ...
    expect(
      client.requests,
      hasLength(4),
      reason: 'two stories, each planned once and written once',
    );
  ...
  test(
    ...
    () async {
      ...
      final invalidBodies = <String, Map<String, Object?>>{
        ...
        'favorite topics': generateRequestBody()
          ..['favoriteTopics'] = 'a' * 241,
        'recurring world': generateRequestBody()..['recurringWorld'] = 7,
      };
